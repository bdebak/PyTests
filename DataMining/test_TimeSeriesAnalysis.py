# -*- coding: utf-8 -*-
"""
Created on Mon Nov 19 23:57:34 2018
...
print(os.listdir("D:/Datasets/Kaggle/time series"))

# %% import
google = pd.read_csv('D:/Datasets/Kaggle/time series/stock-time-series-20050101-to-20171231/GOOGL_2006-01-01_to_2018-01-01.csv', index_col='Date', parse_dates=['Date'])
google.head()

humidity = pd.read_csv('D:/Datasets/Kaggle/time series/historical-hourly-weather-data/humidity.csv', index_col='datetime', parse_dates=['datetime'])
humidity.tail()

# %% cleaning data
humidity = humidity.iloc[1:]
humidity = humidity.fillna(method='ffill') # cleaning missing values
humidity.head()
...
pressure = pd.read_csv('D:/Datasets/Kaggle/time series/historical-hourly-weather-data/pressure.csv', index_col='datetime', parse_dates=['datetime'])
pressure.tail()
...
# psample from 3 day frequency to daily frequency
...
microsoft = pd.read_csv('D:/Datasets/Kaggle/time series/stock-time-series-20050101-to-20171231/MSFT_2006-01-01_to_2018-01-01.csv', index_col='Date', parse_dates=['Date'])
...
# %% window used to identify sub periods, calculates sub-metrics of sub-periods.
...
# %% OHLC charts hows the open, high, low and close price of a certain time period
...
# %% Candlestick charts
# used as a trading tool to visualise and analyse the price movements over time for securities, 
...
# Autoregressive–moving-average (ARMA) models provide a parsimonious description of a (weakly) stationary stochastic process 
...
# Dynamic-factor models are flexible models for multivariate time series in 
# which the observed endogenous variables are linear functions of exogenous covariates and unobserved factors,
# Predicting closing price of Google and microsoft
train_sample = pd.concat([google["Close"].diff().iloc[1:],microsoft["Close"].diff().iloc[1:]],axis=1)
